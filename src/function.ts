...
const answerOriginalFontSize = 45;
const maximumDigitBeforeSizeChange = 8;
const percentageDecrease = 0.1;
let equationFontSizeChange = equationOriginalFontSize;
let answerFontSizeChange = answerOriginalFontSize;
...
const decreaseFontSize = (
  ...
  const residuallengthOfTemporarygth =
    stringlengthOfTemporarygth - maximumDigitBeforeSizeChange;
  for (
    let character = 0;
  ...
  for (let operationButton of operationButtons) {
    operationButton.addEventListener('click', () => {
      if (operationButton.id === 'equals') {
        ...
        answerFontSizeChange = checkDecreaseFontSize(
          outputScreen.value,
          answerOriginalFontSize
        );
        outputScreen.style.fontSize = `${answerFontSizeChange}px`;
      }
    });
  }
}
